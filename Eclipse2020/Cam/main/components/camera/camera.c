...
esp_err_t camera_probe(const camera_config_t* config,
	...
	Debug("Searching for camera address");
	/* Probe the sensor */
	delay(10);
	...
	Debug("Detected camera at address=0x%02x", slv_addr);
	...
	delay(10);
	Debug("Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
	...
	*out_camera_model = CAMERA_OV7725;
	...
	default:
		...
		*out_camera_model = CAMERA_UNKNOWN;
		Debug("Detected camera not supported.")
		;
		return ESP_ERR_CAMERA_NOT_SUPPORTED;
	}

	Debug("Doing SW reset of sensor");
