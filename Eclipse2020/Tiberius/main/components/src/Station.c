...
static void wifi_event(void* arg, esp_event_base_t event_base,int32_t event_id, void* event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        sta->state = CONECTADO_RED;
    }
}

void Conectado_Red(WIFI_EVT e){
	switch(e){
	case COMPLETADO:
...
void Leyendo_SSID(WIFI_EVT e){
   wifi_config_t wifi_config
	...
	int i;
	switch(e){
		case STA_RSSI_PULSOS:

			sta->state=CONECTANDO_RED;
			...
			Debug("Conectando a: %s",(char *)sta->ssid);
			esp_wifi_connect();
			break;
		default:
			break;
...
void Wifi_Station_Task(void *arg){
	...
	while(1)
	{
		if(sta->state == LEYENDO_SSID)
		{
			Debug("Leyendo SSID entry...");
			...
			Debug("numero de redes:%d",ap_number);
			...
			{
				Debug("net num %d ssid:%s rssi:%d",i,(char*)ap_records[i].ssid,ap_records[i].rssi);
				if(strcmp((char*)ap_records[i].ssid,(char*)sta->ssid/*sizeof(ap_records[i].ssid*/)==0){
					Debug("Se encontro red:%s rssi:%d",sta->ssid,ap_records[i].rssi);
		...
		/*############################ CONECTANDO #########################################*/
		if(sta->state==CONECTANDO_RED){};//wait for IPv4 arrend..."
		/*############################ CONECTANDO AL HOST ######################################*/
		if(sta->state==CONECTADO_RED)
		{
			Debug("Conectado a SSID:%s password:%s",(char *)sta->ssid, (char *)sta->psswd);
			State(ENVIANDO);
			i=0;
			...
			if(sto->pWrite == sto->pRead + sto->pPage){
				Conectado_Red(COMPLETADO);
			}
			else{
				Conectado_Red(FALLO_ENVIO);
			}
