...
void Params_Task(void* arg){
	...
	while(1)
	{
		Params_Rx_Events();
		/********** Reset [config] **************/
		if(!(gpio_input_get()&(1ULL<<RESET_INPUT))) //este pin no tiene pull up integrado .-.
		{
			if(reset>=4){
	...
	switch(evt){
		...
		case IDV_WRITE_RCV:
			if(strlen(data)<10){
				...
				printf("+ID: valor demasiado largo, no puede exceder de 9 dígitos\n\n");
			}
			break;
		case SSID_READ_RCV:
			printf("+SSID: %s\n",sta->ssid);
		...
		case URL_WRITE_RCV:
			...
			if(strstr(data,"http://")!=NULL)
			{
				if(Storage_Save_Info("_url#1",data,STRING,PARAM_SECTOR))
					...
					printf("+URL: duplicidad de datos, no se actualizó\n");
			}
			else{
				printf("+URL: valor inválido\n");
			}
...
void Params_Inicio_Evts(PARAMS_EVT evt, char* data){
	...
	switch(evt){
		case CONFIG_RCV:
			...
			printf("Ingrese:\n");
			printf("id? : para saber la placa del vehículo\n");
			printf("id={id} : para cambiar la placa del vehículo\n");
