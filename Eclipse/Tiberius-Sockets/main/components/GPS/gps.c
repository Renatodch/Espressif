    ...
    switch (event_id)
    {
		case GPS_UPDATE:
			...
			Debug("%d/%d/%d %d:%d:%d => \r\n"
					 ...
					 myGPS.tim.hour, myGPS.tim.minute, myGPS.tim.second,
...
void Gps_Init(void){
	...
	myGPS_Parser->parent.altitude = 0.0f;
	myGPS_Parser->parent.longitude = 0.0f;
	myGPS_Parser->parent.latitude = 0.0f;
	myGPS_Parser->parent.speed = 0.0f;
	myGPS_Parser->parent.date.year = 0;
	myGPS_Parser->parent.date.month = 0;
	myGPS_Parser->parent.date.day = 0;
	myGPS_Parser->parent.tim.hour = 0;
	myGPS_Parser->parent.tim.minute = 0;
	myGPS_Parser->parent.tim.second = 0;

	myGPS_Parser->buffer = calloc(1, NMEA_PARSER_RUNTIME_BUFFER_SIZE);
    if (!myGPS_Parser->buffer) ESP_LOGE(GPS_TAG, "calloc memory for runtime buffer failed");

    myGPS_Parser->all_statements |= (1 << STATEMENT_GSA);
    myGPS_Parser->all_statements |= (1 << STATEMENT_GSV);
    myGPS_Parser->all_statements |= (1 << STATEMENT_GGA);
    myGPS_Parser->all_statements |= (1 << STATEMENT_RMC);
    myGPS_Parser->all_statements |= (1 << STATEMENT_GLL);
    myGPS_Parser->all_statements |= (1 << STATEMENT_VTG);
    /* Set attributes */
    myGPS_Parser->uart_port = UART_NUM_2;
    myGPS_Parser->all_statements &= 0xFE;
    /* Install UART friver */
    ...
    if (uart_param_config(myGPS_Parser->uart_port, &uart_config) != ESP_OK)
        ESP_LOGE(GPS_TAG, "config uart parameter failed");

    if (uart_set_pin(myGPS_Parser->uart_port, -1, 16,-1, -1) != ESP_OK)
        ESP_LOGE(GPS_TAG, "config uart gpio failed");

    if (uart_driver_install(myGPS_Parser->uart_port, 1024, 0,16, &myGPS_Parser->event_queue, 0) != ESP_OK)
        ESP_LOGE(GPS_TAG, "install uart driver failed");

    /* Set pattern interrupt, used to detect the end of a line */
    uart_enable_pattern_det_intr(myGPS_Parser->uart_port, '\n', 1, 10000, 10, 10);
    /* Set pattern queue size */
    uart_pattern_queue_reset(myGPS_Parser->uart_port, 16);
    uart_flush(myGPS_Parser->uart_port);
    ...
    if (esp_event_loop_create(&loop_args, &myGPS_Parser->event_loop_hdl) != ESP_OK)
        ESP_LOGE(GPS_TAG, "create event loop failed");

    /* Create NMEA Parser task */
    //BaseType_t err = xTaskCreatePinnedToCore(gps_parser, "nmea_parser",4096,myGPS_Parser,3,&myGPS_Parser->tsk_hdl,0);
    ...
    esp_event_handler_register_with(myGPS_Parser->event_loop_hdl, ESP_NMEA_EVENT, ESP_EVENT_ANY_ID,gps_event_handler, NULL);
}
...
static void esp_handle_uart_pattern(TGPS_Parser *esp_gps)
{
    ...
    if (pos != -1) {
        ...
        //printf((const char *)esp_gps->buffer);
        //printf("\n");
...
{
    esp_gps->parent.tim.hour = convert_two_digit2number(esp_gps->item_str + 0) - 13;
